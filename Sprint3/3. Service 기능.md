...
▶ nodePort로 만들어진 Service 확인<br>
(리눅스 포트 조회 명령으로 볼 수 없음. 실제 리스닝 소켓을 열지 않고, iptables의 규칙을 사용해서 포트를 리디렉션 함)
...
//DNS 호출
sh-4.4# nslookup api-tester-3221-clusterip

//도메인 명으로 curl 날리기
sh-4.4# curl api-tester-3221-clusterip/info
//nslookup api-tester-3221-clusterip 결과의 Address 입력
sh-4.4# curl <address>/info
sh-4.4# curl api-tester-3221-clusterip.anotherclass-322/info
sh-4.4# curl api-tester-3221-clusterip.anotherclass-322.svc.cluster.local/info
```

```
//iptables에서 로드밸런싱
sh-4.4# curl api-tester-3221-clusterip/hostname
```

<img src="images/3/4.png" alt="">

▶ Headless를 통해 Pod의 IP 리스트 확인<br>
...
//nslookup로 확인
sh-4.4# nslookup database-headless
...
▶ Headless를 통해 만들어 진 Pod의 DNS 사용<br>
...
▶ 클러스터 내부의 Pod들이 ExternalName을 통해서 타 서버로 연결<br>
(개발환경에서 테스트 목적으로 다른 환경의 서버(DB or Solution)에 연결을 해야 할 경우)<br>
...
▶ Pod에서 DNS로 호출 시 서버에 질의하지 않고 바로 IP를 설정하는 방법<br>
(프로젝트 초기에 사내 DNS에 내가 필요한 서버에 대한 DNS 등록 안되 있을 경우 사용)
