...
```
[root@k8s-master ~]# while true; do curl http://192.168.56.30:30212/hostname; sleep 0.1; echo ''; done;
```
...
```
[root@k8s-master ~]# kubectl get -n anotherclass-321 -o yaml -w pod <pod_name> 
[root@k8s-master ~]# kubectl logs -n anotherclass-321 -c api-tester-3212 --follow --tail 20 <pod_name>
```
...
[root@k8s-master ~]# kubectl get -n anotherclass-321 pod <pod-name> -o jsonpath='{.status.containerStatuses[*].lastState}'

//previous 옵션을 이용한 이전 컨테이너 로그 20줄만 보기
[root@k8s-master ~]# kubectl logs -n anotherclass-321 <pod-name> -c api-tester-3212 --previous --tail 20
```
